...
# ============================ get the model ==========================
poll="O3";
xVar_Einit=[poll,"erd_$(poll)","erd_$(poll)_Err","$(poll)_Err","PBLH","U","V","windSpeed","windDiv","Ter","Land","TEMP","RH"];
xVar_Efore=[poll,"erd_$(poll)","PBLH","U","V","windSpeed","windDiv","Ter","Land","TEMP","RH"];
xVar_Rfore=[poll,"erd_PM25","erd_AER","erd_ANH4", "erd_ANO3",  "erd_HONO", "erd_ASO4", "erd_NH3", "erd_OC", "erd_HCHO",  "erd_CO", "erd_NO2",
...
modelE=modelE |> gpu ;# 20_328_033 parameters
modelR=modelR |> gpu;
...
# ---------- precompile to save memory usage
...
function mainTrain(tStart_allTrain,modelE,modelR,dataE_info,mInfo,testE_looper;progressBar=true,logFile=logFile,outputFolder=outputFolder)
    opt_E = Adam(0.001,(0.99, 0.9999))
    opt_R = Adam(0.0001,(0.99, 0.9999))
