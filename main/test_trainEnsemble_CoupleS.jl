...
yVarName=poll;
epoch_all=101:110;
xNaqVar_all=[poll,"erd_$(poll)","AER","PBLH","U","V","windSpeed","Ter","Land","TEMP","RH","LEADTIME"];
outT=24*7;
...
# ==================================== training ===============================
function getGradient_modelE(modelEn,X,ensemble,Y_fore,Y_isValid,count)
    loss, gradsE = Flux.withgradient(modelEn) do m
        predEn=m(X,ensemble)
        loss=sum((predEn - Y_fore).^2 .* Y_isValid)/count
    end
    return loss,gradsE
end
