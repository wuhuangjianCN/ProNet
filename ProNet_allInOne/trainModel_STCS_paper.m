function [para_st,para_cs,hf1]=trainModel_STCS_paper(xVarName_st,fileName_dlX_stInit,fileName_dlX_st,fileName_dlY_stTarget,fileName_stLossWeight,para_st,state_st,csDataFun,meshCSInCell,para_cs,option)
    ...
    x_point_cs=1;trailingAvg_cs = [];trailingAvgSq_cs = [];
    ...
    WaitMessage.Send;
        ...
        for n_ind =1:numel(indStart_rand)
            ...
            [gradients_st,gradients_cs,lossAccumulated,dlY_Predicted,dlY_stPredicted] = myGradients_aStart(xVarName_st,dlX_st_init,dlX_st,dlX_cs,dlY_Target,lossWeight,para_st,state_st,para_cs);
            % Update the network parameters using the Adam optimizer.
            [para_st,trailingAvg_st,trailingAvgSq_st] = adamupdate(para_st,gradients_st, trailingAvg_st,trailingAvgSq_st,x_point_st);
            [para_cs,trailingAvg_cs,trailingAvgSq_cs] = adamupdate(para_cs,gradients_cs, trailingAvg_cs,trailingAvgSq_cs,x_point_cs);
            ...
            if plotTraining
                ...
                title(ax1,{"mean Ref:" + mean(lossRef_All,'omitnan') + " ST:" + mean(lossST_All,'omitnan') + " STCS:" + mean(lossSTCS_All,'omitnan'),...
...
function [grad_st,grad_cs,lossAccumulated,dlY_Predicted,dlY_stPredicted] = myGradients_aStart(xVarName,dlX_st_init,dlX_st,dlX_cs,dlY_Target,lossWeight,para_st,state_st,para_cs)
% init ---------------------
%     disp('init...')
    [~,state_st] = model_ST(dlX_st_init,para_st,state_st);
% forecast --------------
    q_before=contains(xVarName,'Before');
    nGradient=0;grad_st=[];grad_cs=[];lossAccumulated=[];
    ...
    for nLead=2:size(dlX_st,5)        
        ...
        if 0==sum(lossWeight_curr) % no valid grid for verify
            ...
            [tmp_grad_st,tmp_grad_cs,state_st,tmp_loss,dlY_predict_current,dlY_st_current] = dlfeval(@myGradients,dlX_st_curr,dlX_cs_curr,dlY_Target_curr, para_st, state_st,para_cs,lossWeight_curr);
            if isnan(tmp_loss);error('nan loss found');end
            if isempty(grad_st)
                grad_st=tmp_grad_st;  grad_cs= tmp_grad_cs;     lossAccumulated=tmp_loss;
            else
                lossAccumulated=tmp_loss+lossAccumulated;
                grad_st=fieldfun(@(x,y) x+y,grad_st,tmp_grad_st);
                for nCell=1:numel(grad_cs);grad_cs{nCell}=fieldfun(@(x,y) x+y,grad_cs{nCell},tmp_grad_cs{nCell});end
            end
    ...
    for nCell=1:numel(grad_cs);grad_cs{nCell}=fieldfun(@(x) x/nGradient,grad_cs{nCell});end
end
...
function loss=myLossForGradient(dlY,dlY_st,dlY_target,weight)
    absLossRatio=0.1;
    loss =sum(weight.*(...
        absLossRatio.*abs(dlY_st-dlY_target).^2+abs(dlY-dlY_target).^2 ...
        ),'all')/sum(weight,'all'); %
end
